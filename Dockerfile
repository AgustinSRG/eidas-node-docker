...
RUN unzip eIDAS-node-dl.zip && \
    ...
    jar -uvf WILDFLY/EidasNodeProxy.war WEB-INF/


RUN apk add wget

RUN wget -P /usr/local/lib https://www.bouncycastle.org/download/bcprov-jdk18on-172.jar 
...
COPY --from=builder --chown=jboss:root /data/WILDFLY/EidasNodeConnector.war /opt/jboss/wildfly/standalone/deployments/EidasNodeConnector.war
COPY --from=builder --chown=jboss:root /data/WILDFLY/EidasNodeProxy.war /opt/jboss/wildfly/standalone/deployments/EidasNodeProxy.war
...
COPY --from=builder --chown=jboss:root /usr/local/lib/bcprov-jdk18on-172.jar  /usr/local/lib/bcprov-jdk18on-172.jar
...
ENV EIDAS_CONNECTOR_CONFIG_REPOSITORY=$EIDAS_CONFIG_REPOSITORY/connector
ENV EIDAS_PROXY_CONFIG_REPOSITORY=$EIDAS_CONFIG_REPOSITORY/proxy
